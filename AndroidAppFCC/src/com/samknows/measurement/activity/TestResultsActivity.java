...
import com.samknows.measurement.test.outparser.OutPutRawParcer;
...
public class TestResultsActivity extends BaseLogoutActivity{
	...
	private void appendDataRow(LinkedList<Holder> data, Comparable<?> value, String displayValues[], boolean isSuccess) {
		int idx = 0;
		for (Holder h : data) {
			if (sortDirection && h.value.compareTo(value) > 0) {
				break;
			} else if (!sortDirection && h.value.compareTo(value) < 0) {
				break;
			}
			idx++;
		}
