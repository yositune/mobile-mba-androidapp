...
public class MainService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		try {
			...
			long maximumTestUsage = config == null ? 0: config.maximumTestUsage*2;
			
			if (appSettings.isServiceEnabled() && LoginHelper.isLoggedIn()) {
				if (!appSettings.isDataCapReached(maximumTestUsage) && !OtherUtils.isRoaming(this)) {
					new StateMachine(this).executeRoutine();
				} else {
					if (appSettings.isDataCapReached(maximumTestUsage)){
						Logger.d(this, "Service disabled(data cap), exiting, test max usage is: "+ maximumTestUsage);
					}
					if (OtherUtils.isRoaming(this)){
						Logger.d(this, "Service disabled(roaming), exiting.");
					}
					OtherUtils.reschedule(this,	Constants.SERVICE_RESCHEDULE_IF_ROAMING_OR_DATACAP);
