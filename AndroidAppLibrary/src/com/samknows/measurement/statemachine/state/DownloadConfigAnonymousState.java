...
public class DownloadConfigAnonymousState extends BaseState {
	...
	public StateResponseCode executeState() {
		...
		if (action.isSuccess()) {
			...
			appSettings.setWakeUpEnabledIfNull(
					config.testAlamType == TestAlarmType.WAKEUP);
			appSettings
					.setLocationTypeIfNull(config.locationType);
			appSettings.saveLong("number_of_tests_schedueld",
					config.getNumberOfScheduledBatch());
			if (config.dataCapDefault >= 0) {
				appSettings.saveDataCapFromConfig(config.dataCapDefault);
			}
