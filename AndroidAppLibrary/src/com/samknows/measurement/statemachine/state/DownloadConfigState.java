...
public class DownloadConfigState extends BaseState{
	...
	public StateResponseCode executeState() {
		...
		if (action.isSuccess()) {
			...
			AppSettings.getInstance().setWakeUpEnabledIfNull(config.testAlamType == TestAlarmType.WAKEUP);
			AppSettings.getInstance().setLocationTypeIfNull(config.locationType);
			AppSettings.getInstance().saveLong("number_of_tests_schedueld", config.getNumberOfScheduledBatch());
			if (config.dataCapDefault >= 0) {
				AppSettings.getInstance().saveDataCapFromConfig(config.dataCapDefault);
			}
